...
CREATE TABLE public.profiles (
  ...
  plaid_inbound_ytd numeric,
  last_plaid_contribution_sync_at timestamp with time zone,
  signup_channel text CHECK (signup_channel = ANY (ARRAY['direct'::text, 'organic_search'::text, 'social'::text, 'referral'::text, 'paid'::text, 'internal'::text])),
...
CREATE TABLE public.hsa_connections (
  ...
  sync_status text NOT NULL DEFAULT 'ok'::text CHECK (sync_status = ANY (ARRAY['ok'::text, 'error'::text, 'login_required'::text])),
  sync_error text,
...
CREATE TABLE public.plaid_transactions (
  ...
  account_id text,
  amount numeric NOT NULL,
  iso_currency_code text NOT NULL DEFAULT 'USD'::text,
  date date NOT NULL,
  name text NOT NULL DEFAULT ''::text,
  merchant_name text,
  pending boolean NOT NULL DEFAULT false,
  reconciliation_status text NOT NULL DEFAULT 'unmatched'::text CHECK (reconciliation_status = ANY (ARRAY['unmatched'::text, 'matched'::text, 'ignored'::text, 'discrepancy'::text])),
  matched_expense_id uuid,
  raw jsonb,
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  updated_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT plaid_transactions_pkey PRIMARY KEY (id),
  CONSTRAINT plaid_transactions_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id),
  CONSTRAINT plaid_transactions_matched_expense_id_fkey FOREIGN KEY (matched_expense_id) REFERENCES public.expenses(id)
);